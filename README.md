...
- **Tiempo real y avisos de escritorio**: la app escucha los cambios de `daily_picks` con Supabase Realtime y se refresca sola. Además hay tres avisos de escritorio: **nueva canción para puntuar** (a todos menos a quien la pone), **te han nominado** (el día que publican la canción que te nomina) y **hoy te toca** (el día de tu turno, una vez al día). Se activan y se eligen en menú de usuario → *Notificaciones*: un interruptor general y uno por tipo, guardados en `localStorage` de cada navegador. Cada aviso llega una sola vez por canción (o por día, el de turno). Funciona mientras la app esté abierta, aunque sea en segundo plano. Con la app cerrada harían falta Web Push y un servidor que los envíe.
...
- **Solo de lunes a viernes**: el turno salta el fin de semana. Si el viernes nominas a alguien, le toca el lunes, y así lo dicen la portada, el aviso de nominación y la tarjeta de Teams. En sábado y domingo la portada muestra *"Es fin de semana"* en vez del formulario, y no salen ni la píldora de turno ni el aviso de *hoy te toca*. Las fechas laborables están en [`src/lib/dates.ts`](src/lib/dates.ts) (`isWorkday`, `nextWorkdayISO`); los festivos no se tienen en cuenta.
